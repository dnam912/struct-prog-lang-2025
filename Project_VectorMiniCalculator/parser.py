...
def parse_program(tokens):
    ...
    while tokens: # Keep parsing while tokens are remained
        if tokens[0]["tag"] == None: # Exit the loop-statement when token is None
            break
        
        statement, tokens = parse_statement(tokens)
        statements.append(statement)
        
        if tokens and tokens[0]["tag"] == ";":  # Check the semicolon
            tokens = tokens[1:]  # Skip the semicolon and move to the next token's position
