...
# dnam - EXTENDED THE CODE FOR RUNNING THE PROJECT
def evaluate(ast, environment):
    ...
    if ast["tag"] == "+":
        ...
        ### vector addition: list + list
        if type(left_value) == list and type(right_value) == list:
            if len(left_value) != len(right_value):
                raise Exception("The size of lists (vectors) is not the same")

            result = []
            i = 0
            while i < len(left_value):
                result.append(left_value[i] + right_value[i])
                i += 1
    ...
    if ast["tag"] == "*":
        ...
        # 1) scalar * list
        if (type(left_value) in [int, float]) and type(right_value) == list:
            result = []
            i = 0
            while i < len(right_value):
                result.append(left_value * right_value[i])
                i += 1
            return result, False
        ...
        ## left_value * right_value -> list * list is not defined here
        return left_value * right_value, False # int * int
